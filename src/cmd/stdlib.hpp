  ...
  Object call(std::vector<Object> args) override {
    ...
    switch (fmt) {
      case FmtOpt::DEC:
        return args[0];
      case FmtOpt::HEX:
        auto val = detail::asAddr<AddrType>(args[1]);
...
#pragma unused(args)
  ...
  static inline FnPtr set =
      requiresRunningTarget([](const std::vector<Object>& args) -> Object {
        auto addr = detail::asAddr<AddrType>(args.front());
...
class DisassembleFn : public Callable {
 public:
  ...
  Object call(std::vector<Object> args) override {
    ...
    if (args.size() > 0)
      if (auto r = detail::asAddr<AddrType>(args[0])) start = *r;
    if (args.size() > 1)
      if (auto r = detail::asAddr<AddrType>(args[1])) end = *r;
